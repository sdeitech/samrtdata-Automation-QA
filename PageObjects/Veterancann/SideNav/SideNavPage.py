...
from PageObjects.smartData2.SideNav.SideNavXpath import SideNavXpath
...
class SideNavPage(BaseHelpers):
    ...
    @allure.step("Click on Change Password from Profile link in side navigation bar")
    def change_profile_password(self):
        try:
            self.mouse_click_action(SideNavXpath.profile_link)
            self.mouse_click_action(SideNavXpath.change_password)
        except Exception as e:
            allure.attach(str(e), name="change_profile_password_exception", attachment_type=allure.attachment_type.TEXT)
            raise
