...
class CalendlyPage(BaseHelpers):
    ...
    @allure.step("Switching to Calendly frame from patient portal")
    def switching_to_calendly_frame_patient_portal(self):
        try:
            self.wait_for_sync(5)
            self.switch_to_iframe(CalendlyXpath.calendly_iframe)
            self.wait_for_sync(5)
        except Exception as e:
            allure.attach(str(e), name="switching_to_calendly_frame_patient_portal_exception", attachment_type=allure.attachment_type.TEXT)
            raise
    ...
    def select_available_slot(self):
        try:
            ...
            if available_dates:
                ...
                self.select_available_date(random_date)

                # Wait for time slots to load and select a random available time slot
                ...
                if available_time_slots:
                    ...
                    self.select_available_time(random_time_slot)
                    self.wait_for_sync(5)
    ...
    @allure.step("Clicking on consultation button")
    def click_consultation(self):
        try:
            self.mouse_click_action(CalendlyXpath.consultation)
            self.wait_for_sync(2)
        except Exception as e:
            allure.attach(str(e), name="click_next_onboard_exception", attachment_type=allure.attachment_type.TEXT)
            raise

    @allure.step("clicking on view button")
    def click_viewbutton(self):
        try:
            self.mouse_click_action(CalendlyXpath.upcomming_drpdwn)
            self.wait_for_sync(2)
        except Exception as e:
            allure.attach(str(e), name="click_next_onboard_exception", attachment_type=allure.attachment_type.TEXT)
            raise

    @allure.step("clicking on calcel button")
    def click_cancelbutton(self):
        try:
            self.mouse_click_action(CalendlyXpath.cancel_btn)
            self.wait_for_sync(2)
        except Exception as e:
            allure.attach(str(e), name="click_next_onboard_exception", attachment_type=allure.attachment_type.TEXT)
            raise
    ...
    @allure.step("Calendly Book Appointment From Patient Portal")
    def calendly_book_appointment_from_patient_portal(self):
        self.click_consultation()

        self.switching_to_calendly_frame_patient_portal()
        self.switching_to_calendly_frame()
        self.switching_to_calendly_frame_2()
        # self.view_next_month()
        self.select_available_slot()
    ...
    @allure.step("calcel appointment from patient portal")
    def cancel_upcomming_appointment(self):
