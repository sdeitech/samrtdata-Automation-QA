...
class PatientNotePage(BaseHelpers):
    ...
    @allure.step("Enter Medicare Number: {medicare_number}")
    def verify_medicare_number(self, medicare_number):
        try:
            xpath = PatientNoteXpath.verify_medicare
            expected = "Ensure this value has at least 10 characters and at most 11 characters (it has 0)."
            self.scroll_into_element(PatientNoteXpath.ihi_number)
            self.wait_for_sync(2)
            self.enter_text_action(medicare_number, PatientNoteXpath.medicare_number)
            self.wait_for_sync(2)
            ...
            allure.attach(str(e), name="verify_medicare_number_exception", attachment_type=allure.attachment_type.TEXT)
            raise
