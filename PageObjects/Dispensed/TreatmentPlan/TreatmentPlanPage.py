...
from PageObjects.Dispensed.TreatmentPlan.TreatmentPlanXpath import TreatmentPlanXpath
from PageObjects.Dispensed.PatientNote.PatientNoteXpath import PatientNoteXpath
...
class TreatmentPlan(BaseHelpers):
    ...
    @allure.step("Fill data for Pathology and save changes")
    def fill_data_and_save_changes(self):
        try:
            self.send_text_action("somedata",TreatmentPlanXpath.textarea_pathology)
            self.mouse_click_action(TreatmentPlanXpath.fasting_xpath)
            self.mouse_click_action(TreatmentPlanXpath.save_changes_xpath)
        except Exception as e:
            allure.attach(str(e), name="filling_data_exception",
                          attachment_type=allure.attachment_type.TEXT)
            raise


    @allure.step("Click on Pathology requested button")
    def click_pathology_requested(self):
        try:
            self.scroll_into_element(TreatmentPlanXpath.pathology_requested_xpath)
            self.wait_for_sync(4)
            self.mouse_click_action(TreatmentPlanXpath.pathology_requested_xpath)
        except Exception as e:
            allure.attach(str(e), name="click_not_prescribed_exception",
                          attachment_type=allure.attachment_type.TEXT)
            raise

    @allure.step("Click on Not prescribed button")
    def click_patient_reviewed_not_prescribed(self):
        try:
            self.scroll_into_element(TreatmentPlanXpath.treatment_not_prescribed)
            self.wait_for_sync(4)
            self.mouse_click_action(TreatmentPlanXpath.treatment_not_prescribed)
        except Exception as e:
            allure.attach(str(e), name="click_not_prescribed_exception",
                          attachment_type=allure.attachment_type.TEXT)
            raise
    ...
    @allure.step("Verify the successfull treatment plan message")
    def verify_treatment_plan_message(self):
        try:
            self.wait_for_sync(5)
            expected = "Successfully approved the Prescription."
            actual = self.get_text_from_element(TreatmentPlanXpath.read_approved_treatment_plan)
            if expected == actual:
                assert True
            else:
                assert False
        except Exception as e:
            allure.attach(str(e), name="get_text_approved_exception",
                          attachment_type=allure.attachment_type.TEXT)
            raise

    @allure.step("Verify the successfully rejected treatment plan message")
    def verify_reject_treatment_plan_message(self):
        try:
            self.wait_for_sync(5)
            expected = "Approval status changed successfully."
            actual = self.get_text_from_element(TreatmentPlanXpath.read_approved_treatment_plan)
            if expected == actual:
                assert True
            else:
            ...
            allure.attach(str(e), name="get_text_rejected_exception",
                          attachment_type=allure.attachment_type.TEXT)
            raise

    @allure.step("Click on Reject Treatment Plan button")
    def click_reject_treatment_plan(self):
        try:
            self.wait_for_sync(5)
            self.scroll_into_element(TreatmentPlanXpath.reject_treatment_plan_btn)
            self.wait_for_sync(4)
            self.mouse_click_action(TreatmentPlanXpath.reject_treatment_plan_btn)
        except Exception as e:
    ...
    @allure.step("Select treatment plan according to pre-defined")
    def select_treatment_plan_defined(self):
        self.click_select_treatment_plan()
        self.click_confirm_btn()

        self.select_select_balance_oil(5)
        self.select_cbd_oil(6)
        self.select_hybrid_vape_cart(7)
        self.select_hybrid_flower(3)
        self.select_indica_vape_cart(5)
        self.select_indica_flower(3)
        self.select_sativa_flower(2)
        self.select_sativa_vape_cart(5)
